...
const verify = async (token, publicKey, algOrOptions) => {
  ...
  let signature;
  try {
    signature = (0, import_encode.decodeBase64Url)(tokenParts[2]);
  } catch {
    throw new import_types.JwtTokenInvalid(token);
  }
  const verified = await (0, import_jws.verifying)(publicKey, alg, signature, import_utf8.utf8Encoder.encode(headerPayload));
  if (!verified) {
