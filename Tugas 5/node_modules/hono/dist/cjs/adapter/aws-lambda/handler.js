...
class EventV1Processor extends EventProcessor {
  ...
  getQueryString(event) {
    if (event.multiValueQueryStringParameters) {
      ...
      return Object.entries(event.queryStringParameters || {}).filter(([, value]) => value !== void 0).map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value || "")}`).join("&");
    }
  }
...
class ALBProcessor extends EventProcessor {
  ...
  getQueryString(event) {
    if (event.multiValueQueryStringParameters) {
      ...
      return Object.entries(event.queryStringParameters || {}).filter(([, value]) => value !== void 0).map(([key, value]) => `${key}=${value}`).join("&");
    }
  }
  getCookies(event, headers) {
    let cookie;
